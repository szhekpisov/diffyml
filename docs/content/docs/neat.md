...
- **`data` / `stringData`** on Secrets — real values; use `--mask-secrets` instead. See [Sensitive Value Masking]({{< relref "/docs/masking" >}}).
...
`--neat-strip-path` appends additional regexes to the neat bundle. For include/exclude filtering independent of `--neat`, see [Filtering & Chroot]({{< relref "/docs/filtering" >}}).
...
CLI flags override config file values. The config file uses positive truth-tables (`helm: false` ⇒ drop helm); the CLI uses opt-out flags (`--no-neat-helm`). See [Configuration File]({{< relref "/docs/config" >}}).
