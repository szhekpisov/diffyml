// color.go - Terminal color detection and built-in ANSI escape codes.
//
// Detects terminal capabilities (color, true color, width) from environment.
// Provides ANSI color codes and the built-in YAML palettes used for diff
// output highlighting. User-customizable overrides live in color_custom.go.
// Key types: ColorMode, ColorConfig, YAMLColorPalette.
package diffyml
