...
// key paths match any regex pattern. Used on the include path where only
// a boolean answer is needed and no hit attribution is required.
...
// whole equal container collapsed in inverse mode). In that case every top-level
// key of the item becomes a nested path, and matching any one of them causes the
// entire diff to be included/excluded. This is intentional — list item diffs and
// inverse-mode subtree collapses are atomic, so partial filtering would not be
// meaningful. Without this, --filter/--exclude on a key nested inside a collapsed
// unchanged subtree would silently never match.
