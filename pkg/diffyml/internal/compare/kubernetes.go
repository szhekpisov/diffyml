...
// Key functions: IsKubernetesResource(), GetK8sResourceIdentifier().
package compare
...
func asK8sResource(doc interface{}) (k8sResource, bool) {
	...
	metaOM := types.ToOrderedMap(metadata)
	if metaOM == nil {
		return k8sResource{}, false
	}
...
// Accepts any value and converts to *types.OrderedMap internally.
func GetIdentifierWithAdditional(v interface{}, additionalIdentifiers []string) interface{} {
	om := types.ToOrderedMap(v)
	if om == nil {
		return nil
	}
...
func GetIdentifierFromOrderedMap(om *types.OrderedMap, additionalIdentifiers []string) interface{} {
...
func MatchK8sDocuments(from, to []interface{}, opts *types.Options) (matched map[int]int, unmatchedFrom, unmatchedTo []int) {
...
func CompareK8sDocs(from, to []interface{}, opts *types.Options, compareFn NodeComparerFn) []types.Difference {
	...
	for _, fromIdx := range remainingFrom {
		...
		pathPrefix := fmt.Sprintf("[%d]", fromIdx)
		diffs = append(diffs, types.Difference{
			Path:          types.CleanPath(pathPrefix),
